	...
	const handleSubmit = async () => {
  	...
  	return (
		<div className = "flex flex-col w-screen h-screen justify-center items-center bg-gradient-to-r from-blue-200 via-green-200 to-purple-300 backdrop-blur-3xl p-4">
			<h1 className = "text-5xl lg:text-7xl font-bold pb-4">FortuneGPT</h1>
			<p className="pb-8">Ask a yes or no question</p>
			<div className = "flex flex-row justify-center items-center space-x-4 p-8">
				<input onChange={e => setPrompt(e.target.value)} className = "w-48 lg:w-72 h-16 border-1 p-4 border border-black rounded-lg" placeholder="Is Sarah really smart?"></input>
				<button type="button" onClick={handleSubmit} className = "w-12 lg:w-24 h-16 flex items-center justify-center border-1 p-4 border border-black rounded-lg bg-gray-200">Ask</button>
			</div>
